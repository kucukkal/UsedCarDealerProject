...
def months_paid_since(sale_date: date, today: date, term_months: Optional[int]) -> int:
...
def build_finance_snapshot(db: Session) -> None:
    ...
    for s in sales_list:
        inv = db.query(models.Inventory).filter(models.Inventory.vin_number == s.vin_number).first()
        cost = inv.cost if inv else 0.0

        sold_dt = s.status_sold_at.date() if s.status_sold_at else None
        sale_date = sold_dt if s.status == "Sold" else None
        ...
        if status == "Sold":
            ...
            if payment_type == "Loan" and s.term_months and s.monthly_payment and sale_date:
                m_paid = months_paid_since(sale_date, today, s.term_months)
                amount_paid = round(m_paid * s.monthly_payment, 2)
                amount_remaining = round(max(0, s.term_months - m_paid) * s.monthly_payment, 2)
    ...
    # Inventory-only cars (not sold, no sales row)
    sales_vins = {s.vin_number for s in sales_list}

    inv_list: List[models.Inventory] = db.query(models.Inventory).filter(models.Inventory.status != "Sold").all()
    for inv in inv_list:
...
def list_customers(
    ...
    rows = db.query(models.Customer).order_by(models.Customer.id.desc()).all()
    return rows
...
def finance_summary(
    ...
    inv_rows: List[models.Inventory] = db.query(models.Inventory).filter(models.Inventory.status != "Sold").all()
    ...
    fin_sold: List[models.Finance] = db.query(models.Finance).filter(models.Finance.status == "Sold").all()
