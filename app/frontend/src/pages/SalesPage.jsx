...
export default function SalesPage() {
    ...
    const [form, setForm] = useState({
        ...
        // NEW: Customer fields (mandatory when status === "Sold")
        customer_name: '',
        customer_zipcode: '',
        customer_city: '',
        customer_state: '',
    ...
    const handleSubmitSale = async (e) => {
        ...
        // NEW: If Sold, customer info is mandatory (backend also enforces this)
        if (form.status === 'Sold') {
            if (!form.customer_name.trim()) return setFormError('Customer Name is required for Sold.')
            if (!form.customer_zipcode.trim()) return setFormError('Customer ZipCode is required for Sold.')
            if (!form.customer_city.trim()) return setFormError('Customer City is required for Sold.')
            if (!form.customer_state.trim()) return setFormError('Customer State is required for Sold.')
            if (!form.customer_phone.trim()) return setFormError('Customer Phone is required for Sold.')
        }
        ...
        // NEW: send customer fields only when Sold
        if (form.status === 'Sold') {
            payload.customer_name = form.customer_name.trim()
            payload.customer_zipcode = form.customer_zipcode.trim()
            payload.customer_city = form.customer_city.trim()
            payload.customer_state = form.customer_state.trim()
        ...
        <div>
            ...
            <section style={{ border: '1px solid #ccc', padding: '1rem' }}>
                ...
                <form onSubmit={handleSubmitSale}>
                    ...
                    {/* Customer fields (mandatory when Sold) */}
                    {form.status === 'Sold' && (
                        <div style={{ marginTop: '1rem', borderTop: '1px solid #ddd', paddingTop: '1rem' }}>
                            <h4 style={{ margin: '0 0 0.5rem 0' }}>Customer Information (Required for Sold)</h4>
